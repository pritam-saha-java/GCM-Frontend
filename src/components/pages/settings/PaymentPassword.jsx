...
import { changePaymentPassword } from "../../../api/userService";
...
export default function PaymentPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await changePaymentPassword(currentPassword, newPassword);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-3xl bg-[#1a1a2f]/70 backdrop-blur-md border border-cyan-500/20 rounded-2xl shadow-2xl text-white p-8">
        <div className="flex items-center gap-3 mb-6">
          <div className="bg-cyan-500/20 p-3 rounded-full">
            <CreditCard size={28} className="text-cyan-400" />
          </div>
          <div>
            <h2 className="text-2xl font-bold">Update Payment Password</h2>
            <p className="text-sm text-gray-400">Secure your payment with a strong password.</p>
          </div>
        </div>
